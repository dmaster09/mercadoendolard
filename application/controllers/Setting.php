	...
	{	
		if ($this->input->post('submit')) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if($result) {
					$this->session->set_flashdata('update_password_success','La contraseña fue actulizada');
					
					redirect(base_url('setting/change_password'));
				}else{
					$this->session->set_flashdata('update_password_failed','La contraseña anterior es incorrecta');
					redirect(base_url('setting/change_password'));
				}
			}
		}

...
class Auth extends Main_Controller {
	...
	public function login()
	{
		...
		{
			//validate inputs
			$this->form_validation->set_rules('email','correo','trim|required|min_length[3]|valid_email' );
	...
	public function registration()
	{
		...
		if($this->input->post()) 
		{
	        ...
	        //validate inputs
			$this->form_validation->set_rules('username','nombre de usuario','trim|required|min_length[5]|is_unique[ci_users.username]');
			
			$this->form_validation->set_rules('email','correo','trim|required|min_length[5]|valid_email|is_unique[ci_users.email]');

			$this->form_validation->set_rules('contact','teléfono','trim|required|is_unique[ci_users.contact]|min_length[7]|max_length[12]');

			$this->form_validation->set_rules('country','país','trim|required');

			$this->form_validation->set_rules('password','contraseña','trim|required|min_length[5]');

			$this->form_validation->set_rules('confirmpassword','confirma contraseña','trim|required|min_length[5]|matches[password]');

			$this->form_validation->set_rules('terms_n_conditions','terminos & condiciones','trim|required');
			...
			else
			{
				...
				$this->session->set_flashdata('success','Hemos enviado un enlace de verificación a tu correo electrónico.');
	...
	public function validate_email($token)
	{
		...
		if($result)
		{
			$this->session->set_flashdata('success','¡Correo verificado! Por favor inicia sesión con tus datos');
			redirect(base_url('login'));
		}
		else
		{
	...
	public function forgot_password()
	{
		...
		if($this->input->post('submit')){
			...
			//validate inputs
			$this->form_validation->set_rules('email', 'correo', 'valid_email|trim|required');
			if ($this->form_validation->run() == FALSE) 
			{
				$data = array(
			...
			if($response){
				...
				if($resp){
					...
					$this->session->set_flashdata('error', 'Hemos enviado instrucciones para restablecer tu contraseña a tu correo electrónico');
